...
from APIs import Server, response_text 
...
def LoadVectors():
    vectors = FAISS.load_local("C:/Users/AUC/Desktop/API testing/gRPC_OpenAI/protos/LLM/AlignedResponsesFiltered_RagDoc", embeddings=embedding, allow_dangerous_deserialization=True) # loading the faiss index
    return vectors
...
# Main Chain
def GetChain():
    conversation = ConversationChain(llm=MainLLM, prompt=prompt)
    chain = RunnableWithMessageHistory(
        runnable=conversation,
...
def Run(UserID, user_input, emotion, background):
    chat = GetChain()
    retrievedText = retrieve_response(user_input)
    retrievedBackground = background
